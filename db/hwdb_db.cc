...
//  config_.kLogPlogSize =
//      config_.kLogMaxWriteSize = 32 * 1024 * 1024;  //
//  config_.kMaxPthreadNum = 8;
//  config_.kPthreadDoFlushJobNum = 5;
//
//  uint64_t nums = stoi(props.GetProperty
//                       (ycsbc::CoreWorkload::RECORD_COUNT_PROPERTY));
//  uint32_t key_len = 24;
//  // stoi(props.GetProperty(CoreWorkload::KEY_LENGTH));
//  uint32_t value_len = stoi(props.GetProperty
//                            (ycsbc::CoreWorkload::FIELD_LENGTH_PROPERTY));
//
...
int HWDB::Scan(const std::string &table, const std::string &key, int len, const std::vector<std::string> *fields,
               std::vector<std::vector<KVPair>> &result) {
  void *iter = nullptr;
  const std::string max_key = "zzzz";
...
//  }
  ...
  std::string whole_value(values[0].second);

  uint64_t pinode = *(uint64_t*)whole_key.data();
  std::string fname = whole_key.substr(8, whole_key.size()-1);

  std::cout << "pinode = " << pinode << ",filename = " << fname << ",stat = "
            << whole_value << endl;

  fflush(stdout);
  s = db_->interface.Putkv(db_->db, Data_S(whole_key).raw_data(), Data_S(whole_value)
                                                              .raw_data());
